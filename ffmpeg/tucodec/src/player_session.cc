...
void VideoProcessor::do_real_work()
{
    ...
    while (true) {
        ...
        } else if (read_next) { /// read next frame
...
//                    lastRePosTS = newts;
           ...
           ////////////+++     glFinish();
                ...
                //reading_ = false;
                setReading(false);
                if (!player.isFetchReady()) { ///< EOS comes before buffering-completed
    ...
    while (true) {
        ...
        //bool output_ready;
        {
            ...
            getCond().wait(lg, [&] () {
                ...
                if (isReading() && !outputsFilled()) { ///< should read more frames
                    read_next = true;
                    return true;
                }
...
//LOG_DEBUG("zzzz ... Bx ");
    ...
    if (!frame) {//FIXME if(!frame && frame->GetTimestamp() + tolerance > audio->GetDuration())
